...
apt-get install php8.1-curl php8.1-fpm php8.1-gd php8.1-mbstring php8.1-mcrypt php8.1-opcache php8.1-xml php8.1-sqlite php8.1-zip php8.1-intl php8.1-mysql php-imagick -y -q
...
wget -O /etc/nginx/conf.d/blacklist.conf https://raw.githubusercontent.com/Pawa-IT-Solutions/lemp-stack-automator/master/blacklist.conf
wget -O /etc/nginx/conf.d/blockips.conf https://raw.githubusercontent.com/Pawa-IT-Solutions/lemp-stack-automator/master/blockips.conf
# Create default file for Nginx for where to find new websites that are pointed to this IP
wget -O /etc/nginx/sites-enabled/default.conf https://raw.githubusercontent.com/Pawa-IT-Solutions/lemp-stack-automator/master/default.conf
# Create fastcgi.conf
...
# Create nginx.conf
wget -O /etc/nginx/nginx.conf https://raw.githubusercontent.com/Pawa-IT-Solutions/lemp-stack-automator/master/nginx.conf
...
sed -i "s/^-l 128.1.0.1/#-l 128.1.0.1/" /etc/memcached.conf
...
sed -i "s/^;pm.max_requests = .*/pm.max_requests = 400/" /etc/php/8.1/fpm/pool.d/www.conf
# State after what amount of time unused PHP-FPM children will stop
sed -i "s/^;pm.process_idle_timeout = .*/pm.process_idle_timeout = 10s;/" /etc/php/8.1/fpm/pool.d/www.conf
...
sed -i "s/^;pm.status_path = \/status/pm.status_path = \/status/" /etc/php/8.1/fpm/pool.d/www.conf
# Create a /ping path for your PHP-FPM installation in order to be able to make heartbeat calls to it
sed -i "s/^;ping.path = \/ping/ping.path = \/ping/" /etc/php/8.1/fpm/pool.d/www.conf
...
sed -i "s/^;opcache.memory_consumption=64/opcache.memory_consumption=64/" /etc/php/8.1/fpm/php.ini
# Raise the maximum limit of variable that can be stored in OPcache
sed -i "s/^;opcache.interned_strings_buffer=4/opcache.interned_strings_buffer=16/" /etc/php/8.1/fpm/php.ini
# Set maximum amount fo files to be cached in OPcache
sed -i "s/^;opcache.max_accelerated_files=2000/opcache.max_accelerated_files=65536/" /etc/php/8.1/fpm/php.ini
# Enabled using directory path in order to avoid collision between two files with identical names in OPcache
sed -i "s/^;opcache.use_cwd=1/opcache.use_cwd=1/" /etc/php/8.1/fpm/php.ini
# Enable validation of changes in php files
sed -i "s/^;opcache.validate_timestamps=1/opcache.validate_timestamps=1/" /etc/php/8.1/fpm/php.ini
# Set validation period in seconds for OPcache file
sed -i "s/^;opcache.revalidate_freq=2/opcache.revalidate_freq=2/" /etc/php/8.1/fpm/php.ini
# Disable comments to be put in OPcache code
sed -i "s/^;opcache.save_comments=1/opcache.save_comments=0/" /etc/php/8.1/fpm/php.ini
# Enable fast shutdown
sed -i "s/^;opcache.fast_shutdown=0/opcache.fast_shutdown=1/" /etc/php/8.1/fpm/php.ini
# Set period in seconds in which PHP-FPM should restart if OPcache is not accessible
sed -i "s/^;opcache.force_restart_timeout=180/opcache.force_restart_timeout=30/" /etc/php/8.1/fpm/php.ini
...
# Monit will check the availability of php8.1-fpm.sock
# And restart php8.1-fpm service if it can't be accessible
# If Monit tries to many times to restart it withour success it will take a timeout and then proceed to restart again
echo -e 'check process php8.1-fpm with pidfile /var/run/php/php8.1-fpm.pid\nstart program = "/etc/init.d/php8.1-fpm start"\nstop program = "/etc/init.d/php8.1-fpm stop"\nif failed unixsocket /run/php/php8.1-fpm.sock then restart\nif 5 restarts within 5 cycles then timeout' > /etc/monit/conf.d/php8.1-fpm.conf
